...
*   **Local Docker Deployment**: The final application container is deployed natively onto the local Docker engine using Ansible.
...
5.  **Continuous Deployment (CD)**: Jenkins delegates the deployment process to **Ansible**. An Ansible playbook (`cd-deploy.yml`) uses the `community.docker.docker_container` module to pull the image and run it directly on the local Docker engine.
